...
from configs.config import EHALL_SERVER_URLS
...
def get_user_info(school_name: str, mod_auth_cas: str) -> tuple[dict, int]:
    ...
    if school_name not in EHALL_SERVER_URLS:
        return {'status': 'error', 'message': f'{school_name} is not supported'}, 400
    ehall_url = EHALL_SERVER_URLS[school_name]
    ...
    if response.status_code != 200:
        return {'status': 'error', 'message': 'Failed to get user info'}, 400
    try:
